import sys
import os
sys.path.insert(1, './work1/okoro/type3detectosra/')  # ensures the local repo version is used
...
def append_daily_csv(hourly_results, date,
    ...
    """
    Append one day's hourly detection results to that year's CSV file.

    Designed to run across an entire solar cycle (up to 11+ years).
    One CSV file is created per calendar year so files remain manageable.
    ...
    CSV layout for each day
    -----------------------
    24 hourly rows, one per hour 00:00 to 23:00, with columns:
        date        | hour  | bursts | raw_groups | samples
        2003-10-27  | 00:00 |   2    |     3      |  3600
        2003-10-27  | 01:00 |   0    |     1      |  3598
        ...
        2003-10-27  | 23:00 |   1    |     2      |  3601

    Followed immediately by one summary row for that day:
        2003-10-27  | TOTAL |  15    |    25      |  86399

    Then the next day starts directly below the TOTAL row:
        2003-10-28  | 00:00 |  ...

    This structure means:
      - filter df[df.hour != 'TOTAL']  → pure hourly data across all days
      - filter df[df.hour == 'TOTAL']  → one row per day for day-level stats
      - filter df[df.date == '2003-10-27'] → full record for one day

    Parameters
    ----------
    hourly_results : list of 24 dicts
        One dict per hour in order 0–23, each containing:
            'hour'       : int  0–23
            'bursts'     : int  confirmed burst groups (len > 1)
            'raw_groups' : int  all line groups including single-line
            'samples'    : int  number of time records in that hour
        Built inside the hour loop of read_wholeday_v1 / v2.

    date : str
        ISO date string for this observation day, e.g. "2003-10-27".
        The year is extracted from this string to route to the correct file.
    ...
    Returns
    -------
    str : full path of the CSV file that was written to.
    """

    # ── Extract year from the date string to select the correct yearly file ───
    year     = date.split("-")[0]                              # e.g. "2003"
    filename = f"bursts_{year}.csv"
    filepath = os.path.join(output_dir, filename)             # e.g. outputs/solar_cycle/bursts_2003.csv

    os.makedirs(output_dir, exist_ok=True)

    # ── Build the 24 hourly rows ───────────────────────────────────────────────
    hourly_rows = []
    for entry in hourly_results:
        hourly_rows.append({
            "date"       : date,
            "hour"       : f"{entry['hour']:02d}:00",   # format as "00:00" … "23:00"
            "bursts"     : entry["bursts"],
    ...
    # ── Build the daily total row ──────────────────────────────────────────────
    total_row = pd.DataFrame([{
        "date"       : date,
        "hour"       : "TOTAL",
        "bursts"     : hourly_df["bursts"].sum(),
        "raw_groups" : hourly_df["raw_groups"].sum(),
        "samples"    : hourly_df["samples"].sum()
    }])

    # ── Combine: 24 hourly rows then 1 total row ───────────────────────────────
    day_block = pd.concat([hourly_df, total_row], ignore_index=True)

    # ── Append to the yearly CSV (write header only if file is new) ───────────
    file_is_new = not os.path.exists(filepath)

    day_block.to_csv(
        filepath,
        mode    = "a",               # append — never overwrite existing data
        header  = file_is_new,       # write column names only on the very first write
        index   = False
    )

    print(f"  [{date}] written → {filepath}  "
          f"({hourly_df['bursts'].sum()} bursts, "
          f"{hourly_df['raw_groups'].sum()} groups, "
          f"{hourly_df['samples'].sum()} samples)")
